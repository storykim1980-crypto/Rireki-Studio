...
function buildRirekiA4(){
  ...
  const nameRow = h('div',{class:'r-row name', style:'grid-template-columns:1fr'},
    h('div',{class:'c', style:'flex-direction:column;align-items:flex-start;gap:.5mm'},
  ...
  const idRows = h('div',{class:'r-rows'}, nameRow, birthRow);
...
function fitA4(a4El, fitEl){
  ...
  const scale = Math.min(1, w / mmToPx(210));
...
function bindPreviewFit(){
  const ro = new ResizeObserver(()=>{ fitA4($('a4Preview'),$('pvFit1')); fitA4($('a4Preview2'),$('pvFit2')); fitA4($('a4PreviewT'),$('pvFit3')); fitA4($('a4PreviewS'),$('pvFit4')); });
  ...
  const fi = $('impFile');
    ...
    reader.onload = ()=>{
      try{
        ...
        if (!parsed || typeof parsed !== 'object' || !('profile' in parsed)) throw new Error('형식 오류');
        store.replace(sanitizeState(parsed));          // 스키마 정제 후 교체
        fillProfileForm();
        toast('バックアップを復元しました');
      }catch(e){ console.warn(e); toast('対応していないファイルです', 'error'); }
    };
...
function init(){
  try{
    ...
    bindPreviewFit();
    bindSettings();
    bindPhotoUpload(); bindCrop(); bindEditor();
    bindLibrary();
    bindDiag();
    bindMensetsu();
    bindGyaku();
    ...
    /* 초기 렌더 */
    fillProfileForm();
    bindTaishoku(); fillTaishokuForm();
    bindSofu(); fillSofuForm();
    renderDynamic();
    applyTheme();
    setStep(1);
